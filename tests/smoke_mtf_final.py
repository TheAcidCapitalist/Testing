...
with Storage("data/scanner_test.duckdb") as storage:
    ...
    for t, e in tickers:
        ...
        # daily
        s = box_breakout.compute_series(prices, lookback=60, touch_tolerance=0.05, compression_threshold=1.5, duration_pct=0.7)
        counts["daily"] += s["direction"].isin(["buy", "sell"]).sum()
        
        # weekly
        w = resample_ohlcv(prices, "W-FRI")
        if not w.empty:
            s = box_breakout.compute_series(w, lookback=104, touch_tolerance=0.15, compression_threshold=1.5, duration_pct=0.7)
            counts["weekly"] += s["direction"].isin(["buy", "sell"]).sum()
            
        # monthly
        m = resample_ohlcv(prices, "ME")
        if not m.empty:
            s = box_breakout.compute_series(m, lookback=240, touch_tolerance=0.30, compression_threshold=1.5, duration_pct=0.7)
